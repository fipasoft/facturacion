...
class gacl {
	...
	*/
	...
	function gacl($options = NULL) {
		...
		// load Kumbia options from /forms/config/config.ini
		$acl = Config::read('acl.ini');
		foreach ($acl->options as $key => $value) {
			$this->debug_text("Kumbia Option: $key");

			if (in_array($key, $available_options) ) {
				$this->debug_text("Valid Kumbia Config options: $key");
				$property = '_'.$key;
				$this->$property = $value;
			} else {
				$this->debug_text("ERROR: Kumbia Config option: $key is not a valid option");
			}
		}
