 ...
 */
 ...
 class XLSFactura extends Plantilla{

    protected $path;
 	protected $simbolos;

 	public function XLSFactura( $facturas ){
 	    ...
 	    if( count( $facturas ) == 1 ){
 	        ...
 	        $f = $facturas[ 0 ];
     		$this->Plantilla( 'Factura-' . $f->verFolio() . '.xls' );

 	    }else{

 	        $this->Plantilla( 'Factura.xls' );
    ...
    private function emisor( &$h, $factura ){
        ...
        $h->cc = 7;
        $h->xls->write( $h->rr, $h->cc, 'FACTURA', $st['A11c'] );
	...
	private function inicializarVariables(){
	 	...
	 	// paths
	 	$path = new stdClass();
	 	$path->xml = getcwd() . '/app/reportes/factura.xml';
...
//	    echo 'Memoria usada: ' . round(memory_get_usage() / 1024,1).' KB<br/>';
	...
	private function totales( &$h, $factura ){
        ...
        $h->xls->insertBitmap($h->rr, 0, getcwd() . '/public/img/system/qr.bmp', 0, 0, 0.48, 0.52);
        ...
        $leyenda = array(
          (
            $factura->leyendaDeEfectosFiscales() ?
            Session :: get_data( 'fsc.leyenda' ) :
            ''
          ),
          'La reproducción apócrifa de este comprobante constituye un delito ' .
          'en los términos de las disposiciones fiscales',
          'Este comprobante tendrá una vigencia de dos años contados a ' . 
          'partir de la fecha de aprobación de la asignación de folios, la ' . 
          'cual es ' . Session :: get_data( 'fsc.faprobacion' ),
          'Folio inicial: ' . Session :: get_data( 'fsc.folioini' ) . ' ' . 
          'Folio final: ' . Session :: get_data( 'fsc.foliofin' ) . '  ' . 
          'Número de Aprobación:  ' . Session :: get_data( 'fsc.naprobacion' )
        );

        foreach( $leyenda as $i => $linea ){
